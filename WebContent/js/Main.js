...
var Map= new Map();
var currentStreetLat;
var currentStreetLng;
...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        */
        ...
        /**Map.zoom slider
         * ZOOM FUNCTION ACTIVE WHEN CHANGE ZOOM_SLIDER
         */ 
        $("#zoom_slider").on("change", function (e, val) {
        	console.log("Zoom_slider changed");
            start = new Date().getTime();
            if(graphtype_flag==1&&(val / 20 + 12==13)){
            	location.reload();
            }
            else{
...
function centerCamera() {
    ...
    camera.rotation.y = 0;
    camera.rotation.z = 0;
    camera.rotation.x = -0.7853981633974483 ;
}
    ...
    if ($("#streetSearchBox").val() != "") {
        ...
        for (var index = 0; index < jsonObj.streetStats.length; index++) {
            if (jsonObj.streetStats[index].formatted_address == $("#streetSearchBox").val()) {
                ...
                currentStreetLat=Map.centerLat;
                currentStreetLng=Map.centerLng;
...
function drawDistAllGraph() {
    ...
    console.log("cam position x:"+camera.position.y);
    console.log("cam position x:"+camera.position.z);
    console.log("cam rotation x:"+camera.rotation.x);
    console.log("cam rotation x:"+camera.rotation.y);
    console.log("cam rotation x:"+camera.rotation.z);
    */
	
}

function drawDistDatatypeGraph() {	
	console.log("drawDistDatatypeGraph()");
    //$("input[type='checkbox'][name='datatype']").prop('checked', true);	
    //$("#datatype_all").prop('checked', false);
    Map.centerLat = 21.0357;
    Map.centerLng = 105.8261;
    ...
    case 12:
        //BAR_HEIGHT_MULTIPLE= 1;
    	//console.log("Map.zoom=12,13: Map.TILE_MULTIPLE = 9");
        //Map.TILE_MULTIPLE = 9;
        console.log(' centerLat: '+ Map.centerLat);
        break;
    case 14:
    case 15:
    case 16:
    case 17:
        //BAR_HEIGHT_MULTIPLE= 0.9;
    	console.log("Map.zoom=14-17: Map.TILE_MULTIPLE = 11");
        Map.TILE_MULTIPLE = 11;
        break;
    }
...
/**INITIALIZE THREE JS OBJECT : SCENE, CAMERA, LIGHT
 * 
 */
function init() { 
	console.log("init()");
...
/**INITIALIZE SCENE SURROUNDING : DOME, FLOOR
 * 
 */
function createScene() {
    console.log("createScene()");
    skyDome = new THREE.Mesh(new THREE.SphereGeometry(DOME_RADIUS * 120 / 100, 32, 15, 0, Math.PI * 2), new THREE.MeshLambertMaterial({
        map: THREE.ImageUtils.loadTexture('images/sky_lightblue_1000.jpg'),
        side: THREE.BackSide
    }));
...
function graph() {	
    ...
    case 1:
        ...
        */
        ...
        for (var index = 0; index < jsonObj.districtStats.length; index++) {
            ...
            if ($("input[type='checkbox'][value=" + jsonObj.districtStats[index].district_id + "][name='dist']").is(':checked') && Math.abs(Map.x_coord) < Map.STATIC_MAP_SIZE * Map.TILE_MULTIPLE / 2 && Math.abs(Map.y_coord) < Map.STATIC_MAP_SIZE * Map.TILE_MULTIPLE / 2) {
    ...
    case 2:
        ...
        for (var index = 0; index < jsonObj.dataTypeStats.length; index++) {
            ...
            for (var datatype_index = 1; datatype_index <= 4; datatype_index++) {
                ...
                if ($("input[type='checkbox'][value=" + datatype_index + "][name='datatype']").is(':checked') && $("input[type='checkbox'][value=" + jsonObj.dataTypeStats[index - 1].district_id + "][name='dist']").is(':checked') && Math.abs(Map.x_coord) < Map.STATIC_MAP_SIZE * Map.TILE_MULTIPLE / 2 && Math.abs(Map.y_coord) < Map.STATIC_MAP_SIZE * Map.TILE_MULTIPLE / 2) {
    ...
    case 3:
        ...
        for (var index = 0; index < jsonObj.streetStats.length; index++) {
            ...
            if ((Math.abs(Map.x_coord) < Map.STATIC_MAP_SIZE * Map.TILE_MULTIPLE / 2 && Math.abs(Map.y_coord) < Map.STATIC_MAP_SIZE * Map.TILE_MULTIPLE / 2) && (max_height < jsonObj.streetStats[index].count + 0.1)) {
	...
	*/
        ...
        for (var index = 0; index < jsonObj.streetStats.length; index++) {
            ...
            for (var datatype_index = 1; datatype_index <= 4; datatype_index++) {
                ...
                if ($("input[type='checkbox'][value=" + datatype_index + "][name='datatype']").is(':checked') && $("input[type='checkbox'][value=" + jsonObj.streetStats[index].district_id + "][name='dist']").is(':checked') && (Math.abs(Map.x_coord) < Map.STATIC_MAP_SIZE * Map.TILE_MULTIPLE / 2) && (Math.abs(Map.y_coord) < Map.STATIC_MAP_SIZE * Map.TILE_MULTIPLE / 2)) {
                    count++;
    ...
    case 4:
        ...
        for (var index = 0; index < jsonObj.streetStats.length; index++) {
            ...
            Map.latLngToCoord(jsonObj.streetStats[index].lat, jsonObj.streetStats[index].lng);
            if ($("input[type='checkbox'][value=" + jsonObj.streetStats[index].district_id + "][name='dist']").is(':checked') && Math.abs(Map.x_coord) < Map.STATIC_MAP_SIZE * Map.TILE_MULTIPLE / 2 && Math.abs(Map.y_coord) < Map.STATIC_MAP_SIZE * Map.TILE_MULTIPLE / 2) {
                count++;
                ...
                for (var i = 0; i < geometry.faces.length; i++) {
                	if(jsonObj.streetStats[index].lat==currentStreetLat
                			&&jsonObj.streetStats[index].lng==currentStreetLng){
